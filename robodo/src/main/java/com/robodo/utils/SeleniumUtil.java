...
public class SeleniumUtil {
	...
	public void startWebDriver() {
		...
		String fileDownloadPath = this.runnerUtil.getEnvironmentParameter("file.download.path");
	...
	public void executeJavascript(WebElement el, String javaScript, String arguments) {
		...
		Object[] valuesExtracted = Splitter.on(",").trimResults().splitToList(arguments).stream()
				.filter(p -> p != null && !p.isBlank()).map(p -> {
	...
	public void waitElementAttributeToBe(By xpath, String attribute, String expectedValue) {
		runnerUtil.logger(
				"wait attribute [%s] of element to be [%s]: for element %s".formatted(attribute, expectedValue, xpath));
	...
	public void waitPageLoaded() {
		...
		while (true) {
			JavascriptExecutor executor = (JavascriptExecutor) webDriver;
			boolean readyStateComplete = ((String) executor.executeScript("return document.readyState"))
					.equals("complete");
	...
	public void focusWithTab(WebElement el) {
		...
		while (true) {
			...
			try {
				getWebDriverWait().withTimeout(Duration.ofMillis(100))
	...
	public void switchToNextNewTab() {
		
		switchToNextNewTab(getWindowHandle());
	}
	
	public void switchToNextNewTab(String currentWindowHandle) {
		List<String> tabs = new ArrayList<String>(webDriver.getWindowHandles());
		Optional<String> nextTab = tabs.stream().filter(p->!p.equals(currentWindowHandle)).findFirst();
		if (nextTab.isEmpty()) {
			throw new RuntimeException("No new tab/window found to switch to");
		}
		webDriver.switchTo().window(nextTab.get());

	}
	
	public void switchToFirstTab() {
		List<String> tabs = new ArrayList<String>(webDriver.getWindowHandles());
		Optional<String> firstTab = tabs.stream().findFirst();
		if (firstTab.isEmpty()) {
			throw new RuntimeException("No new tab/window found to switch to");
		}
		webDriver.switchTo().window(firstTab.get());
